"""
Tableau MCP ChatBotの簡易テストスクリプト
"""
...
from bedrock.mcp_chatbot_http import MCPChatBotHTTP
...
async def test_chatbot():
    """ChatBotの基本機能をテスト"""
    print("=== Tableau MCP ChatBot Test ===\n")

    # 環境変数から設定を読み込む
    try:
        bedrock_config = BedrockConfig.from_env()
        tableau_config = TableauMCPHttpConfig.from_env()
    except ValueError as e:
        print(f"Error: {e}")
        return

    # ChatBotを初期化
    chatbot = MCPChatBotHTTP(bedrock_config, tableau_config)
    ...
    print("1. Connecting to Tableau MCP Server...")
